...
def dense_dot_product(q_dense, p_dense):
    return (q_dense * p_dense).sum(dim=1)


class DenseSparseDistilKLLoss(Loss):
    """
    ...
    def __init__(self, q_regularizer=None, d_regularizer=None, normalize="softmax", reg_ent=True, anti_zero=False) -> None:
        ...
        """
        super(DenseSparseDistilKLLoss, self).__init__(
    ...
    def forward(self, q_reps, p_reps, n_reps, labels):
        ...
        """
        q_tok_ent_ids, q_tok_ent_weights, q_dense = q_reps
        p_tok_ent_ids, p_tok_ent_weights, p_dense = p_reps
        n_tok_ent_ids, n_tok_ent_weights, n_dense = n_reps
        ...
        pos_rel_dense = dense_dot_product(q_dense, p_dense)
        neg_rel_dense = dense_dot_product(q_dense, n_dense)
        dense_scores = torch.stack([pos_rel_dense, neg_rel_dense], dim=1)
        dense_scores = torch.log_softmax(dense_scores, dim=1)
        if self.reg_ent:
            reg_q_output = (
        ...
        sparse_kl_loss = self.loss(student_scores, teacher_scores).sum(
            dim=1).mean(dim=0)
        dense_kl_loss = self.loss(
            dense_scores, teacher_scores).sum(dim=1).mean(dim=0)
        kl_loss = (sparse_kl_loss + dense_kl_loss)/2
