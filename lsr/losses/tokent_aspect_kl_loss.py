...
def sparse_dot_product(q_tok_ids, q_tok_weights, q_ent_ids, q_input_ids,  q_ent_mask, q_ent_logits, d_tok_ids, d_tok_weights, d_ent_ids, d_input_ids, d_ent_mask, d_ent_logits):
    batch_size = q_ent_ids.size(0)
    entity_match_mask = (q_ent_ids.unsqueeze(-1) == d_ent_ids.unsqueeze(-2))
    tok_match_mask = (q_input_ids.unsqueeze(-1) ==
                      d_input_ids.unsqueeze(-2))
    ...
    d_ent_logits = d_ent_logits[batch_indices,
                                d_tok_indices]
    entity_interaction = (q_ent_logits * d_ent_logits * mask)
    if entity_interaction.dim() > 1:
        entity_interaction = entity_interaction.sum(dim=-1)
    entity_scores = torch.zeros(batch_size, device=q_ent_ids.device)
    entity_scores = entity_scores.scatter_add(
        0, batch_indices, entity_interaction)
    tok_interaction = (q_tok_weights.unsqueeze(-1) * d_tok_weights.unsqueeze(-2))*((q_tok_ids.unsqueeze(-1) ==
                                                                                    d_tok_ids.unsqueeze(-2)))
    tok_scores = tok_interaction.max(dim=-
                                     1).values.sum(dim=-1)
    scores = entity_scores + tok_scores
    return scores
...
class TokentAspectKLLoss(Loss):
    ...
    def __init__(self, q_regularizer=None, d_regularizer=None, normalize="softmax") -> None:
        ...
        """
        super(TokentAspectKLLoss, self).__init__(q_regularizer, d_regularizer)
    ...
    def forward(self, q_reps, p_reps, n_reps, labels):
        ...
        """
        q_tok_ids, q_tok_weights, q_ent_ids, q_input_ids, q_ent_mask, q_ent_logits = q_reps
        p_tok_ids, p_tok_weights, p_ent_ids, p_input_ids, p_ent_mask, p_ent_logits = p_reps
        n_tok_ids, n_tok_weights, n_ent_ids, n_input_ids, n_ent_mask, n_ent_logits = n_reps
        ...
        p_rel = sparse_dot_product(
            q_tok_ids, q_tok_weights, q_ent_ids, q_input_ids, q_ent_mask, q_ent_logits, p_tok_ids, p_tok_weights, p_ent_ids, p_input_ids, p_ent_mask, p_ent_logits)
        # similarity with positive documents
        n_rel = sparse_dot_product(
            q_tok_ids, q_tok_weights, q_ent_ids, q_input_ids, q_ent_mask, q_ent_logits, n_tok_ids, n_tok_weights, n_ent_ids, n_input_ids, n_ent_mask, n_ent_logits)
        ...
        if self.q_regularizer is not None:
            reg_q_output = self.q_regularizer(q_tok_weights)
            + self.q_regularizer(q_ent_logits.view(q_ent_logits.size(0), -1))
        reg_d_output = torch.tensor(0.0, device=q_ent_ids.device)
        if self.d_regularizer is None:
            reg_d_ouptut = self.d_regularizer(p_tok_weights)
            + self.d_regularizer(n_tok_weights)
            + self.d_regularizer(p_ent_logits.view(q_ent_logits.size(0), -1))
            + self.d_regularizer(n_ent_logits.view(n_ent_logits.size(0), -1))
            reg_d_ouptut = reg_d_ouptut * 0.5  # average betweenn two documents
        ...
        to_log = {
            "query_reg": reg_q_output.detach(),
            "doc_reg": reg_d_output.detach(),
            "query_#tokens": num_non_zero(q_tok_weights),
            "doc_#tokens": num_non_zero(p_tok_weights),
            "query_#entities": num_non_zero(q_ent_logits.view(q_ent_logits.size(0), -1)),
            "doc_#entities": num_non_zero(p_ent_logits.view(p_ent_logits.size(0), -1)),
