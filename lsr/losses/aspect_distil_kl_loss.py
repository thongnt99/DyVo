...
def sparse_dot_product(q_tok_ent_ids, q_tok_ent_weights, q_aspect_vectors, d_tok_ent_ids, d_tok_ent_weights, d_aspect_vectors):
    # batch_size x q_len
    # batch_size x d_len
    exact_match_mask = q_tok_ent_ids.unsqueeze(
        -1) == d_tok_ent_ids.unsqueeze(1)
    importance_match = q_tok_ent_weights.unsqueeze(-1) * \
        d_tok_ent_weights.unsqueeze(1)
    aspect_match = q_aspect_vectors @ d_aspect_vectors.transpose(2, 1)
    interaction = importance_match * aspect_match * exact_match_mask
    dot_product = interaction.sum(dim=-1).sum(dim=-1)
    return dot_product


class AspectDistilKLLoss(Loss):
    """
    ...
    def __init__(self, q_regularizer=None, d_regularizer=None, normalize="softmax") -> None:
        ...
        """
        super(AspectDistilKLLoss, self).__init__(q_regularizer, d_regularizer)
    ...
    def forward(self, q_reps, p_reps, n_reps, labels):
        ...
        """
        q_tok_ent_ids, q_tok_ent_weights, q_aspect_vectors = q_reps
        p_tok_ent_ids, p_tok_ent_weights, p_aspect_vectors = p_reps
        n_tok_ent_ids, n_tok_ent_weights, n_aspect_vectors = n_reps
        ...
        p_rel = sparse_dot_product(
            q_tok_ent_ids, q_tok_ent_weights, q_aspect_vectors, p_tok_ent_ids, p_tok_ent_weights, p_aspect_vectors)
        # similarity with positive documents
        n_rel = sparse_dot_product(
            q_tok_ent_ids, q_tok_ent_weights, q_aspect_vectors, n_tok_ent_ids, n_tok_ent_weights, n_aspect_vectors)
